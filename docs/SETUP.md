...
**Дедуп:** каждый PR ревьюится **один раз**. Новые коммиты сами ревью НЕ перезапускают —
никаких авто-перепрогонов.

## Перепрогнать ревью (реран)

Внизу каждого обзора есть чекбокс **`🔄 Re-run review`**. Поставь галку (нужен write-доступ к PR)
— при ближайшем проходе бот снимет её и запостит **новый** обзор-коммент (старый остаётся в
истории PR). Латентность — до ~15 мин (cron). Команда-коммент `/rerun-review` для сторонних репо
появится позже.
