...
1. Триггеры: **мгновенно по вебхуку** (открытие PR → реле → `repository_dispatch`), **cron ~15 мин**
   как подстраховка, кнопка **Actions → app-review → Run workflow** и **клик по реран-ссылке** в
   обзоре. Вебхук-реле опционально (без него — только cron/кнопка); см. `relay/README.md`.
2. **discover** — выпускает токен App'а, обходит репо из `REPO_ALLOWLIST`, берёт открытые PR,
   пропускает уже отревьюенные (по нашему обзор-коммену в PR), заводит Check Run `in_progress`.
...
**Дедуп:** каждый PR ревьюится **один раз**. Новые коммиты сами ревью НЕ перезапускают — никаких
авто-перепрогонов.

## Перепрогнать ревью (реран)

Внизу каждого обзора — ссылка **🔄 Re-run this review**. Клик → реле выпускает `repository_dispatch`
→ бот постит **новый** обзор-коммент (старый остаётся в истории PR). Работает при подключённом
сервере-реле (переменная `RELAY_URL` + секрет `RERUN_SECRET`); без него ссылки в обзоре нет.
Подключение сервера — **[relay/README.md](../relay/README.md)**.
