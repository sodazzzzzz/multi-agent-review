defmodule Aggregator.RenderTest do
  ...
  describe "walkthrough (LLM-обзор PR)" do
    ...
    test "когорта: заголовок + сводка + пофайловая таблица + валидный mermaid" do
      wt = %{
        ...
        groups: [
          %{
            title: "Refunds",
            summary: "guard zero",
            files: [%{path: "lib/refund.ex", change: "guard amount==0"}]
          }
        ],
      ...
      assert out.summary =~ "**Refunds** — guard zero"
      assert out.summary =~ "| File | Change |"
      assert out.summary =~ "<code>lib/refund.ex</code>"
      assert out.summary =~ "guard amount==0"
      assert out.summary =~ "```mermaid\nflowchart TD\nA-->B\n```"
    end

    test "несколько когорт → несколько заголовков и таблиц" do
      wt = %{
        tldr: "T.",
        groups: [
          %{title: "API", summary: "new route", files: [%{path: "router.ex", change: "+route"}]},
          %{
            title: "Tests",
            summary: "coverage",
            files: [%{path: "router_test.exs", change: "+test"}]
          }
        ],
        mermaid: nil
      }
      ...
      out = Render.render(Cluster.build([finding(line: 1)]), %{walkthrough: wt})
      assert out.summary =~ "**API** — new route"
      assert out.summary =~ "**Tests** — coverage"
      assert out.summary =~ "<code>router.ex</code>"
      assert out.summary =~ "<code>router_test.exs</code>"
    end

    test "безымянная когорта (fallback) → только таблица, без жирного заголовка" do
      wt = %{
        tldr: "T.",
        groups: [%{title: "", summary: "", files: [%{path: "a.ex", change: "x"}]}],
        mermaid: nil
      }

      out = Render.render(Cluster.build([finding(line: 1)]), %{walkthrough: wt})
      assert out.summary =~ "| File | Change |"
      assert out.summary =~ "<code>a.ex</code>"
      refute out.summary =~ "** —"
    end

    test "невалидный mermaid отбрасывается, tldr остаётся" do
      wt = %{tldr: "T.", groups: [], mermaid: "not a diagram, just prose"}
      out = Render.render(Cluster.build([finding(line: 1)]), %{walkthrough: wt})
      assert out.summary =~ "<summary>📝 Walkthrough</summary>"
    ...
    test "mermaid с ```-забором отбрасывается (не ломает блок)" do
      wt = %{tldr: "T.", groups: [], mermaid: "flowchart TD\nA-->B\n```evil"}
      out = Render.render(Cluster.build([finding(line: 1)]), %{walkthrough: wt})
      refute out.summary =~ "```mermaid"
    end
    ...
    test "`|` в ячейке таблицы экранируется сущностью (не ломает таблицу)" do
      wt = %{
        tldr: "T.",
        groups: [%{title: "G", summary: "", files: [%{path: "a.ex", change: "do a | b"}]}],
        mermaid: nil
      }
    ...
    test "тройной ``` в tldr/change глушится сущностью (не открывает код-забор)" do
      wt = %{
        tldr: "Done. ```",
        groups: [%{title: "G", summary: "", files: [%{path: "a.ex", change: "see ```js"}]}],
        mermaid: nil
      }
