...
- **Само-ревью этого репо** — событийно на каждый PR (`review.yml`; см. «Включение пайплайна» ниже).
- **Сторонние репо «из коробки»** — центральный **GitHub App** ревьюит открытые PR в доверенных
  репозиториях из `REPO_ALLOWLIST`, выполняясь здесь и постя обзор + нативный Check Run от лица бота
  (`multi-agent-review[bot]`). В целевых репо ничего не настраивается, секреты централизованы, сервер
  не нужен. Полный гайд: **[docs/SETUP.md](docs/SETUP.md)**.
...
```yaml
- uses: sodazzzzzz/multi-agent-review@v0.2.0
  with:
...
| `pr-number` | — | номер PR |
| `target-repo` | `""` | целевой `owner/repo` для постинга; пусто → `GITHUB_REPOSITORY` (центральный App-режим) |
